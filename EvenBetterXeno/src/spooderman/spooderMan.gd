...
func checkRays() -> void:
	...
	var avgPos : Vector3
	var amtOfRaysColliding := 0
	for ray in $head/rayFolder.get_children():
		var r : RayCast = ray
		if r.is_colliding():
			amtOfRaysColliding += 1
			avgPos += r.get_collision_point()
			avgNor += r.get_collision_normal()
	if avgNor:
		avgPos /= amtOfRaysColliding
		avgNor /= amtOfRaysColliding
...
func get_dir() -> Vector3:
	...
	if Input.is_action_pressed("ui_up"):
		dir = avgNormal.cross( fowardDir ).normalized().rotated( avgNormal.normalized(), -PI/2 )

	if Input.is_action_pressed("ui_down"):
		dir = avgNormal.cross( fowardDir ).normalized().rotated( avgNormal.normalized(), PI/2 )

	if Input.is_action_pressed("ui_left"):
		dir = avgNormal.cross( fowardDir ).normalized()

	if Input.is_action_pressed("ui_right"):
		dir = avgNormal.cross( fowardDir ).normalized().rotated(avgNormal.normalized(), PI)
